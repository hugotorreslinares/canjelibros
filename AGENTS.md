...
- `ModerationView` (route `moderation`, header link visible only to moderators) lists **every** book with owner name and reservation state, a text filter, an inline edit form, and delete. It's a content-policy tool, not a shelf: it bypasses slot limits and skips the thread cleanup that `ShelfView`'s delete performs, so moderating away a book leaves any open trade around it untouched (the confirm dialog says so).
